...
// Extracted outside FeedPage so it isn't recreated every render
...
// Memoized card — only re-renders when its specific auction data changes
const AuctionCard = memo(({ auction, summary, commentCount, store, patchStore, meUser, onNavigate }) => {
  ...
  return (
    <div className="feed-card" onClick={() => onNavigate("auction", auction.id)}>
      ...
      <div className="card-image">
        {auction.imageUrl ? <img src={auction.imageUrl} alt={auction.title} loading="lazy" /> : <span>{auction.emoji || "🎨"}</span>}
      ...
      <div className="card-body">
        ...
        <div className="card-ooh-row feed-ooh">
          ...
          <WatchButton auctionId={auction.id} store={store} patchStore={patchStore} meUser={meUser} onNavigate={onNavigate} />
        </div>
      </div>
    </div>
  );
...
const FeedPage = ({ onNavigate, store, updateStore, patchStore, me, meCollector }) => {
  ...
  const meUser = me || meCollector;

  const followingIds = useMemo(
    () => new Set(store.collectors?.[meCollector?.id]?.following || []),
    [store.collectors, meCollector?.id]
  );
  const hasFollowing = !!(meCollector && followingIds.size > 0);

  const live = useMemo(
    () => store.auctions.filter((a) => !a.removed && a.published && getStatus(a) === "live"),
    [store.auctions]
  );
  ...
  const filtered = useMemo(() => {
    ...
    return sorted;
  }, [sorted, sort, followingIds]);

  const visibleAuctions = filtered.slice(0, visibleCount);
  const hasMore = filtered.length > visibleCount;

  // "Because you follow" strip
  const forYouDrops = useMemo(() => {
    if (sort === "following" || !hasFollowing) return [];
    return live.filter(a => followingIds.has(a.artistId))
      .sort((a, b) => new Date(a.endDate) - new Date(b.endDate))
      .slice(0, 8);
  }, [live, sort, hasFollowing, followingIds]);
              ...
              return (
                <div key={auction.id} className="for-you-card" onClick={() => onNavigate("auction", auction.id)}>
                  <div className="for-you-img">
                    {auction.imageUrl ? <img src={auction.imageUrl} alt={auction.title} loading="lazy" /> : <span style={{ fontSize:"2rem" }}>{auction.emoji || "🎨"}</span>}
                  </div>
                ...
                <div className="coming-soon-body">
                  ...
                  <div onClick={e => e.stopPropagation()} style={{ marginTop: "0.5rem" }}>
                    <WatchButton auctionId={auction.id} store={store} patchStore={patchStore}
                      meUser={meUser} onNavigate={onNavigate} />
                  </div>
                </div>
              </div>
            ))}
          ...
          <div className="feed-grid">
            {visibleAuctions.map(auction => (
              <AuctionCard
                key={auction.id}
                auction={auction}
                summary={store.bidSummaries[auction.id] || { count: 0, topAmount: 0 }}
                commentCount={store.commentCounts?.[auction.id] || 0}
                store={store}
